...
class JNFExp(EnhancementExp):
    ...
    def get_init_params(_config):
        model = FTJNF(**_config['network'])
        stft_length = _config['data']['stft_length_samples']
        stft_shift = _config['data']['stft_shift_samples']
        init_params = {
            'model': model,
    ...
    def shared_step(self, batch, batch_idx, stage: Literal['train', 'val']):
        ...
        self.log(f'{stage}/loss', loss, on_step=on_step, on_epoch=True, logger=logger_enabled, sync_dist=True)
        self.log(f'{stage}/clean_td_loss', clean_td_loss.mean(), on_step=on_step, on_epoch=True, logger=logger_enabled, sync_dist=True)
        self.log(f'{stage}/noise_td_loss', noise_td_loss.mean(), on_step=on_step, on_epoch=True, logger=logger_enabled, sync_dist=True)
        self.log(f'{stage}/clean_mag_loss', clean_mag_loss.mean(), on_step=on_step, on_epoch=True, logger=logger_enabled, sync_dist=True)
        self.log(f'{stage}/noise_mag_loss', noise_mag_loss.mean(), on_step=on_step, on_epoch=True, logger=logger_enabled, sync_dist=True)
        if batch_idx < 1:
        ...
        if stage == 'val' or stage == 'test':
            self.log(f'monitor_loss', loss, on_step=on_step, on_epoch=True, logger=logger_enabled)
            global_si_sdr = self.compute_global_si_sdr(est_clean_td, clean_td)
            stage_prefix = 'val' if stage == 'val' else 'test'
            self.log(f'{stage_prefix}/si_sdr', global_si_sdr.mean(), on_epoch=True, logger=logger_enabled, sync_dist=True)
