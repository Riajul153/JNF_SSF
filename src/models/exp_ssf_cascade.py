...
class SSFCascadeExp(EnhancementExp):

    def __init__(
        self,
        model: nn.Module,
        post_enhancer: nn.Module,
        learning_rate: float,
        weight_decay: float,
        loss_alpha: float,
        stft_length: int,
        ...
        super(SSFCascadeExp, self).__init__(
            model=model,
        ...
        self.model = model
        self.post_enhancer = post_enhancer
        self._post_window = None
    ...
    def _get_post_window(self):
        if self._post_window is None or self._post_window.device != self.device:
            self._post_window = torch.sqrt(torch.hann_window(self.stft_length, device=self.device))
        return self._post_window

    def apply_post_enhancer(self, clean_td: torch.Tensor):
        win = self._get_post_window()
        spec = torch.stft(
            clean_td,
            self.stft_length,
            self.stft_shift,
        ...
        post_spec = self.post_enhancer(spec_real)
        post_complex = torch.view_as_complex(post_spec.contiguous())
        post_td = torch.istft(
            post_complex,
            self.stft_length,
            self.stft_shift,
        ...
        orig_len = clean_td.shape[-1]
        if post_td.shape[-1] != orig_len:
            if post_td.shape[-1] > orig_len:
                post_td = post_td[..., :orig_len]
            else:
                pad = orig_len - post_td.shape[-1]
                post_td = torch.nn.functional.pad(post_td, (0, pad))
        return post_td
    ...
    def shared_step(self,
        ...
        est_clean_td = self.apply_post_enhancer(est_clean_td)
